...
/**
 * The reply the scripted assistant gives to a typed or spoken message. This is
 * the text/voice half of the demo: it is intentionally shallow — a handful of
 * canned, on-topic answers keyed off obvious intent — because the demo badge is
 * lit and nothing here is real reasoning. It stays bilingual so an Arabic
 * session is answered natively rather than translated.
 */
const REPLIES: Record<Locale, { recipe: string; shopping: string; expiry: string; greet: string; generic: string }> = {
  en: {
    ...
    greet: 'Hi! I can suggest a recipe, build a shopping list, or tell you what is expiring. What would help?',
    generic:
...
export class MockRealtimeAssistantClient implements RealtimeAssistantClient {
  ...
  /** Monotonic turn id source, so every reply gets a unique React key. */
  private idSeq = 0;
  ...
  async start({ locale, stream, onEvent }: StartAssistantOptions): Promise<void> {
    ...
    const hasCamera = (stream?.getVideoTracks?.().length ?? 0) > 0;
