...
export interface VerifiedIdentity {
  ...
  email: string | null;
  /**
   * The person's own name as the provider spells it, used only to seed
   * `display_name` on first sign-in. Null whenever the provider did not supply
   * one — Apple puts the name in the authorization credential rather than the
...
export class OAuthService implements OnModuleInit {
  ...
  /**
   * Accepts a comma-separated list because one client id never covers every
   * platform: Apple mints `aud` as the bundle id for a native iOS sign-in but
   * as the Services ID on the web, and Google issues a separate client id per
   * platform. Pinning a single value would lock out every platform but one.
   */
