...
function stubGoogleTokeninfo(
  ...
  name: string | typeof ABSENT = ABSENT,
): void {
  vi.stubGlobal(
    'fetch',
    vi.fn(async () => ({
...
describe('OAuthService audience pinning', () => {
  ...
  it('returns the name Google reports so the account can be created with it', async () => {
    const clientId = 'my-client.apps.googleusercontent.com';
    stubGoogleTokeninfo(clientId, 'true', 'Abed Omar');
    const service = new OAuthService(envWith({ GOOGLE_CLIENT_ID: clientId }));
  ...
  it('treats a blank Google name as absent rather than an empty display name', async () => {
    const clientId = 'my-client.apps.googleusercontent.com';
  ...
  it('accepts any client id in a comma-separated list, so every platform can sign in', async () => {
    const ios = 'ios-client.apps.googleusercontent.com';
    const web = 'web-client.apps.googleusercontent.com';
    const service = new OAuthService(envWith({ GOOGLE_CLIENT_ID: ` ${ios} , ${web} ` }));

    for (const clientId of [ios, web]) {
      stubGoogleTokeninfo(clientId);
      await expect(service.verify('google', 'header.body.sig')).resolves.toMatchObject({
        providerAccountId: 'google-sub-123',
      });
  ...
  it('still rejects a client id absent from the list', async () => {
    stubGoogleTokeninfo('attacker-client.apps.googleusercontent.com');
    const service = new OAuthService(
      envWith({
        GOOGLE_CLIENT_ID: 'ios-client.apps.googleusercontent.com,web-client.apps.googleusercontent.com',
      }),
    );
